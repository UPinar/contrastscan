...
class TestCBinary:
    ...
    def test_c_binary_no_crash(self, scanner_path, fuzz_input):
        result = subprocess.run([str(scanner_path), fuzz_input], capture_output=True, text=True, timeout=3)
    ...
    def test_malicious_ns_in_dig_safe(self, malicious_ns):
        ...
        try:
            result = subprocess.run(
                ["dig", f"@{malicious_ns}", "example.com", "AXFR", "+short"], capture_output=True, text=True, timeout=3
            )
