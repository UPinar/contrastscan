...
class TestScanFlow:
    def test_post_scan_redirects(self):
        with patch("scanner.run_scan", side_effect=mock_run_scan), \
...
class TestCsrfE2e:
    ...
    def test_valid_origin_allowed(self):
        with patch("scanner.run_scan", side_effect=mock_run_scan), \
             patch("scanner.validate_domain", side_effect=mock_validate_domain):
            r = client.post("/scan", data={"domain": "example.com"},
    ...
    def test_api_json_has_11_modules(self):
        with patch("scanner.run_scan", side_effect=mock_run_scan), \
            ...
            assert all(k in data for k in
                       ["headers", "ssl", "dns", "redirect", "disclosure", "cookies", "dnssec",
                        "methods", "cors", "html", "csp_analysis"])
...
class TestRateLimitE2e:
    def test_ip_hourly_limit_triggers_429(self):
        ...
        with patch("scanner.run_scan", side_effect=mock_run_scan), \
             patch("scanner.validate_domain", side_effect=mock_validate_domain):
            r = client.post("/scan", data={"domain": "example.com"},
                            follow_redirects=False, headers=CSRF_HEADERS)
...
class TestEnterpriseE2e:
    def test_enterprise_note_visible_for_google(self):
        ...
        with patch("scanner.run_scan", return_value=mock_google), \
             patch("scanner.validate_domain", side_effect=mock_validate_domain):
...
class TestBadge:
    ...
    def setup_badge_data(self):
        ...
        if not get_scan("a" * 32):
            save_scan("a" * 32, "badge-test.com",
                      {"grade": "A", "total_score": 100}, "A", 100)
...
class TestXssPrevention:
    ...
    def test_xss_in_domain_escaped_on_result_page(self):
        ...
        result_data = {
            "domain": xss_domain,
            "total_score": 50, "max_score": 100, "grade": "C",
            ...
            "dnssec": {"score": 0, "max": 5, "details": {"dnssec_enabled": False}},
            "methods": {"score": 5, "max": 5, "details": {"trace_enabled": False, "delete_enabled": False, "put_enabled": False}},
            "cors": {"score": 0, "max": 5, "details": {"wildcard_origin": False, "reflects_origin": False, "credentials_with_wildcard": False}},
            "html": {"score": 0, "max": 5, "details": {"mixed_active": 0, "mixed_passive": 0, "inline_scripts": 0, "inline_handlers": 0, "external_scripts": 0, "external_scripts_no_sri": 0, "forms_total": 0, "forms_http_action": 0}},
...
class TestApiErrorLeakage:
    ...
    def test_invalid_api_key_no_hash_leak(self):
        r = client.get("/api/scan?domain=example.com",
                        headers={"Authorization": "Bearer csc_invalid123"})
...
class TestBadgeSvgInjectionE2e:
    ...
    def test_badge_with_xss_grade_in_db_sanitized(self):
        ...
        save_scan("d" * 32, "xss-badge.com",
                  {"grade": "<img>", "total_score": 50}, "<img>", 50)
...
class TestContentDispositionE2e:
    ...
    def test_report_filename_safe_characters_only(self):
        ...
        with patch("main.get_recon", return_value={"status": "done", "result": "{}"}):
            ...
            assert all(c in "abcdefghijklmnopqrstuvwxyz0123456789.-" for c in filename.replace("-security-report.txt", ""))
